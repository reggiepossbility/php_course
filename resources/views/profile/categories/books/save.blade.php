        ...
        <tbody id="myTable">
            @foreach ($books->reverse() as $book)
                <tr class="row">
                    ...
                    <td class="col-2" >{{ $book->time }}</td>
                    <td class="col" >{{ Str::limit($book->detail, 70, $end = '...') }}</td>
                    <td class="col-1" > <a href="{{ route('books.show', $book) }}" class="btn btn-outline-success">看看</a></td>
                </tr>
            @endforeach
        </tbody>
    </table>
